...
inline static int final_cond(int _out_1_x0,int _out_1_x3,int _out_3_x0,int _out_3_x3) {
  switch (_out_1_x0) {
  case 1:
    switch (_out_1_x3) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:X0=%i; 1:X3=%i; 3:X0=%i; 3:X3=%i;\n",c,show ? '*' : ':',(int)o[out_1_x0_f],(int)o[out_1_x3_f],(int)o[out_3_x0_f],(int)o[out_3_x3_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("IRIW+addrs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("IRIW+addrs, check_globals failed");
  }
...
"ldr %w[x3],[%[x4],%w[x2],sxtw]\n"
"#END _litmus_P3\n\t"
:[x3] "=&r" (out_3_x3[_i]),[x0] "=&r" (out_3_x0[_i]),[x2] "=&r" (trashed_x2)
:[x1] "r" (&_a->y[_i]),[x4] "r" (&_a->x[_i])
:"cc","memory"
);
  }
  mbar();
...
static void init(ctx_t *_a) {
  ...
  _a->out_3_x3 = malloc_check(size_of_test*sizeof(*(_a->out_3_x3)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int _out_3_x3_i = ctx.out_3_x3[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_1_x0_i,_out_1_x3_i,_out_3_x0_i,_out_3_x3_i));
      o[out_1_x0_f] = _out_1_x0_i;
      o[out_1_x3_f] = _out_1_x3_i;
      o[out_3_x0_f] = _out_3_x0_i;
      o[out_3_x3_f] = _out_3_x3_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test IRIW+addrs Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "IRIW+addrs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
