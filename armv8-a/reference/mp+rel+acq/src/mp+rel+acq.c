...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("mp+rel+acq, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("mp+rel+acq, check_globals failed");
  }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test mp+rel+acq Allowed\n");
  ...
  fprintf(out,"Hash=d634f4a4e53b49ec70069a662efe7c5d\n");
  ...
  fprintf(out,"Observation mp+rel+acq %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  if (p_true > 0) {
  }
  fprintf(out,"Time mp+rel+acq %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "mp+rel+acq: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
